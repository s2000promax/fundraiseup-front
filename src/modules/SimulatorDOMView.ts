...
export class SimulatorDOMView implements View {
  ...
  registerSymbolClickHandler(handler: (char: string, index: number) => void) {
    this.handleClickEvent = handler;
  }

  registerKeyboardClickHandler(handler: (key: KeyboardEvent) => void) {
  ...
  renderStatistics(statParam1: number, statParam2: number, statParam3: string) {
